...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const tableService = new TableService({
        ...
        renderRow: (item) => {
            ...
            const payoutDate = item.payout_date ? new Date(item.payout_date).toLocaleDateString() : '—';
    ...
    const formService = new FormService({
        ...
        buildUrl: (formData) => {
            ...
            return { url: `/admin/disbursements/${id}`, isEdit: true };
        },
        onSaved: () => {
    ...
    window.processPayout = async (id) => {
        ...
        try {
            // We use the already fetched table data if available, or fetch fresh
            // For simplicity, we fetch fresh
            const response = await fetch(`/admin/disbursements?id=${id}`);
            ...
            if (data) {
                ...
                summaryContainer.innerHTML = `
                    <div class="row g-2">
                        <div class="col-6"><span class="text-muted">Payee:</span> <span class="fw-bold">${data.application?.user?.name}</span></div>
                        <div class="col-6"><span class="text-muted">Type:</span> <span class="fw-bold">${data.renewal_id ? 'Renewal' : 'Initial Grant'}</span></div>
                        <div class="col-12"><span class="text-muted">Program:</span> <span class="fw-bold">${data.application?.scholarship?.name}</span></div>
                    </div>
                `;
            }
        } catch (error) {
